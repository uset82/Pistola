...
3. **Concept (gate 1).** Use `$imagegen` (built-in `image_gen`) to make 2 or 3 distinct variants following [references/concept-prompting.md](references/concept-prompting.md). Show them and ask which one to build or what to change. **Stop and wait for the user's answer.** Iterate with one targeted change per round.
...
7. **Build.** Execute the chunks in order on the best available surface. After each chunk, read the result; on an error, fix that chunk only, and never replay chunks that already succeeded.
