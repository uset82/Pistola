...
1. MCP default tools (`pistola_status`, `pistola_task_*`, `pistola_run`, `pistola_check`, `pistola_inspect`, `pistola_export_scene`, `pistola_screenshot`).
...
4. For each part: `inspect` → `validate` → `taskPlan.runStep` → `waitForIdle` → `inspect` / `exportScene` / `pistola_check` / `screenshot`. `run` and `runStep` already embed `{structure}`.
5. At most **2 retries per step** from the issue list (`fix.patch` when present). Then fall back to a simpler technique. Do not retry the same failing action. Opt-in `strict: true` reverts a regression; `taskPlan.restoreBest` / `pistola_task_restore_best` reloads the best snapshot.
