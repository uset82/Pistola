...
def get_default_freecad_cmd_candidates() -> tuple[Path, ...]:
    ...
    return (
        ...
        freecad_root / "build" / "Debug" / "bin" / "FreeCADCmd.exe",
        freecad_root / "build" / "bin" / "FreeCADCmd.exe",
        freecad_root / ".pixi" / "envs" / "default" / "Library" / "bin" / "FreeCADCmd.exe",
    )


def get_default_freecad_cmd_path() -> Path:
    for candidate in get_default_freecad_cmd_candidates():
        resolved_candidate = candidate.resolve()
        if resolved_candidate.exists() and resolved_candidate.is_file():
            return resolved_candidate

    return get_default_freecad_cmd_candidates()[0].resolve()


def get_freecad_cmd_path(env: dict[str, str] | None = None) -> Path:
    raw_path = _trimmed_env("FREECAD_PATH", env)
    candidate_path = (
        Path(raw_path).expanduser() if raw_path else get_default_freecad_cmd_path()
    )
    resolved_path = candidate_path.resolve()
    if not resolved_path.exists() or not resolved_path.is_file():
        if not raw_path:
            searched_paths = ", ".join(
                str(candidate.resolve()) for candidate in get_default_freecad_cmd_candidates()
            )
            raise FreeCADRuntimeError(
                "FREECAD_PATH is not set and no integrated FreeCAD executable was found. "
                f"Checked: {searched_paths}. Build editor/third_party/FreeCAD or set "
                "FREECAD_PATH explicitly."
            )
        raise FreeCADRuntimeError(
