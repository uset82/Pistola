...
function PropertyLineSection() {
  ...
  const handleToggleEdit = () => {
    void runAssistantCommand([{ type: 'set_mode', mode: isEditing ? 'select' : 'edit' }])
  }
  ...
  const handlePointChange = (index: number, axis: 0 | 1, value: number) => {
    ...
    void runAssistantCommand([{ type: 'update_site_properties', nodeId: siteNode.id, polygon: newPoints }])
  }
        ...
        <div className="flex flex-col gap-0.5">
          {hasCamera && (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation()
                void runAssistantCommand([{ type: 'view_camera_snapshot', nodeId }], {
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation()
              void runAssistantCommand([{ type: 'capture_camera_snapshot', nodeId }], {
                onSuccess: () => onOpenChange(false),
              })
            }}
          >
          ...
          {hasCamera && (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation()
                void runAssistantCommand([{ type: 'clear_camera_snapshot', nodeId }], {
                  onSuccess: () => onOpenChange(false),
                })
              }}
            >
...
function LevelReferences({
  ...
  const handleDelete = async (nodeId: string, e: React.MouseEvent) => {
    ...
    if (selectedReferenceId === nodeId) {
      setSelectedReferenceId(null)
    }
    void runAssistantCommand([{ type: 'delete_target', nodeId }])
  }
    ...
    <div className="relative flex flex-col">
      ...
      >
          ...
          <button
            ...
            onClick={(e) => {
              ...
              if (isSelected) {
                ...
                void runAssistantCommand([{ type: 'focus_level', levelId: level.id }])
              }
            }}
          >
            {isExpanded ? (
        ...
        <div
          ...
          onClick={() => void runAssistantCommand([{ type: 'focus_level', levelId: level.id }])}
          onDoubleClick={() => setIsEditing(true)}
        >
          <img
            alt="Level"
            ...
            <div className="flex flex-col gap-0.5">
              {level.camera && (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    void runAssistantCommand([{ type: 'view_camera_snapshot', nodeId: level.id }], {
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  void runAssistantCommand([{ type: 'capture_camera_snapshot', nodeId: level.id }], {
                    onSuccess: () => setCameraPopoverOpen(false),
                  })
                }}
              >
              ...
              {level.camera && (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    void runAssistantCommand([{ type: 'clear_camera_snapshot', nodeId: level.id }], {
                      onSuccess: () => setCameraPopoverOpen(false),
                    })
                  }}
                >
          ...
          <PopoverContent align="start" className="w-40 p-1" side="right">
            {level.level !== 0 && (
              <button
                ...
                onClick={() => void runAssistantCommand([{ type: 'delete_target', nodeId: level.id }])}
              >
                <Trash2 className="h-3.5 w-3.5" />
                Delete
              </button>
...
function LevelsSection({
  ...
  const handleAddLevel = () => {
    void runAssistantCommand([{ type: 'create_level', buildingId: building.id }])
  }
    ...
    <div className="pointer-events-none sticky top-4 z-50 flex h-0 w-full justify-center overflow-visible">
      <div className="pointer-events-auto flex items-center gap-2.5 rounded-full border border-primary/20 bg-primary px-0.5 py-4 pl-2 font-medium text-primary-foreground text-xs shadow-black/10 shadow-lg backdrop-blur-md">
        ...
        <button
          ...
          onClick={() => void runAssistantCommand([{ type: 'select_nodes', nodeIds: [] }])}
    ...
    >
      ...
      >
        <button
          ...
          onClick={() => {
            void runAssistantCommand([
              { type: 'focus_building', buildingId: building.id },
              ...(phase === 'site' ? ([{ type: 'set_phase', phase: 'structure' }] as const) : []),
            ])
          }}
        >
          <img
            ...
            <div className="flex flex-col gap-0.5">
              {building.camera && (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    void runAssistantCommand([{ type: 'view_camera_snapshot', nodeId: building.id }], {
                      onSuccess: () => setBuildingCameraOpen(null),
                    })
                  }}
                >
                  <Camera className="h-3.5 w-3.5" />
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  void runAssistantCommand([{ type: 'capture_camera_snapshot', nodeId: building.id }], {
                    onSuccess: () => setBuildingCameraOpen(null),
                  })
                }}
              >
              ...
              {building.camera && (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    void runAssistantCommand([{ type: 'clear_camera_snapshot', nodeId: building.id }], {
                      onSuccess: () => setBuildingCameraOpen(null),
                    })
                  }}
                >
      ...
      <div className="flex h-full flex-col">
        ...
        {siteNode && (
          <motion.div
            ...
            layout="position"
            onClick={() => void runAssistantCommand([{ type: 'set_phase', phase: 'site' }])}
          >
            <div className="flex items-center gap-2">
              <img
                alt="Site"
