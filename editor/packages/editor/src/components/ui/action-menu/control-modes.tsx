        ...
        return (
          <ActionButton
            className={cn(
              ...
              m.id === 'build' && isActive && 'ring-1 ring-green-400/40',
            )}
            key={m.id}
            label={m.label}
            onClick={() => void runAssistantCommand([{ type: 'set_mode', mode: m.id }])}
            shortcut={m.shortcut}
            size="icon"
            tooltipContent={
              m.id === 'build' ? <p>Build Mode (B) unlocks authoring tools.</p> : undefined
            }
            variant="ghost"
          >
            {m.imageSrc ? (
