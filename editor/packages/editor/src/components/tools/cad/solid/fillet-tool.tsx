...
export const FilletTool: React.FC = () => {
  ...
  const applyFilletToSelection = useCad((state) => state.applyFilletToSelection)
  ...
  const hint = body
    ? 'Choose one or more preview edges, then queue a fillet regeneration.'
    : 'Select a single CAD body before applying a fillet.'
  const statusMessage =
    lastError ||
    (helperUnavailable
      ? cadHelperUnavailableMessage
      : canApply
        ? `Queue fillet on ${selectedEdgeRefs.join(', ')}.`
        : hint)
      ...
      <div className="pointer-events-none fixed top-24 left-1/2 z-50 w-[380px] -translate-x-1/2">
        <div className="pointer-events-auto rounded-2xl border border-border/60 bg-background/95 p-3 shadow-2xl backdrop-blur-xl">
          ...
          <button
            ...
            onClick={() => void applyFilletToSelection({ edgeRefs: selectedEdgeRefs, radius })}
            type="button"
          >
            <Box className="h-4 w-4" />
            Confirm Fillet
