...
const selectReference = (node: AnyNode) => {
  ...
  if (nodeLevelId !== 'default' && nodeLevelId !== viewer.selection.levelId) {
    updates.levelId = nodeLevelId
  }
  if (buildingId && buildingId !== viewer.selection.buildingId) {
    updates.buildingId = buildingId
  }

  viewer.setSelection(updates)
...
const SELECTION_STRATEGIES: Record<string, SelectionStrategy> = {
  ...
  structure: {
    types: ['wall', 'item', 'zone', 'slab', 'ceiling', 'roof', 'window', 'door', 'scan', 'guide'],
  ...
  cad: {
    types: ['cad-sketch', 'cad-body', 'scan', 'guide'],
    handleSelect: (node, nativeEvent, modifierKeys) => {
      if (node.type === 'scan' || node.type === 'guide') {
        selectReference(node)
        return
      ...
      if (node.type === 'cad-sketch') {
        useScene.getState().updateNode(node.id, { editStatus: 'editing' } as Partial<AnyNode>)
        useEditor.getState().setActiveSketchId(node.id)
      } else {
...
export const SelectionManager = () => {
  ...
  useEffect(() => {
    ...
    const onDoubleClick = (event: NodeEvent) => {
      ...
      let targetPhase: 'site' | 'structure' | 'furnish' | 'cad' | null = null
...
const EditorOutlinerSync = () => {
  ...
  useEffect(() => {
    ...
    switch (phase) {
      case 'site':
        ...
        if (selectedReferenceId) idsToHighlight = [selectedReferenceId]
      ...
      case 'structure':
        ...
        idsToHighlight = selectedReferenceId ? [selectedReferenceId] : selection.selectedIds
        break
      ...
      default:
        ...
        if (selectedReferenceId) idsToHighlight = [selectedReferenceId]
        else if (selection.selectedIds.length > 0) idsToHighlight = selection.selectedIds
  ...
  }, [phase, selectedReferenceId, selection, hoveredId, outliner])
