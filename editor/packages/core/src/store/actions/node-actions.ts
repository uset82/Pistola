...
export const updateNodesAction = (
  ...
  const bodiesToRegen = new Set<AnyNodeId>()
  const sketchFieldsThatAffectBodies = new Set([
    'plane',
    'planeAnchorNodeId',
    'position',
    'rotation',
  ...
  set((state) => {
    ...
    for (const { id, data } of updates) {
      ...
      if (
        currentNode.type === 'cad-sketch' &&
        Object.keys(data).some((key) => sketchFieldsThatAffectBodies.has(key))
      ) {
        for (const candidate of Object.values(nextNodes)) {
          if (
            candidate.type === 'cad-body' &&
            (candidate.sourceSketchId === id || candidate.sourceSketchIds.includes(id))
          ) {
            if (
              candidate.regenStatus === 'building' ||
              candidate.regenStatus === 'queued' ||
              candidate.regenStatus === 'running' ||
              candidate.regenStatus === 'pending'
            ) {
              continue
            }
            ...
            bodiesToRegen.add(candidate.id)
          }
        }
      }
    }
  ...
  requestAnimationFrame(() => {
    ...
    bodiesToRegen.forEach((bodyId) => {
      get().markDirty(bodyId)
    })
