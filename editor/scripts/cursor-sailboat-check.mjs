...
const wslHost = () => {
  const result = spawnSync('wsl', ['-d', 'Ubuntu', '--', 'bash', '-lc', 'ip route show default'], {
    encoding: 'utf8',
  })
  const host = result.stdout.match(/via\s+(\d+\.\d+\.\d+\.\d+)/)?.[1]
  if (!host) throw new Error(`Could not read the WSL host address: ${result.stderr}`)
  return host
}
...
const runCursor = () => new Promise((resolve) => {
  const child = spawn('wsl', [
    '-d', 'Ubuntu', '--',
    '/home/carlos/.local/bin/cursor-agent',
    '-p',
    '--force',
    '--approve-mcps',
    '--trust',
    '--sandbox', 'disabled',
    '--workspace', '/mnt/d/Proyectos/pistolacodex',
    prompt,
  ], {
...
const main = async () => {
  ...
  const host = wslHost()
  console.log(`stand-in ${standIn.sessionId} via ${host}`)
  const pinned = {
    mcpServers: {
      pistola: {
        command: 'node',
        args: ['/mnt/d/Proyectos/pistolacodex/editor/tooling/pistola-mcp/src/index.ts'],
        env: {
          PISTOLA_TARGET: 'local',
          PISTOLA_TRANSPORT: 'bridge',
          PISTOLA_BASE_URL: `http://${host}:3002`,
  ...
  try {
    await writeFile(mcpPath, `${JSON.stringify(pinned, null, 2)}\n`)
    result = await runCursor()
  } finally {
    await writeFile(mcpPath, originalMcp)
  ...
  if (best && best.size > 10_000) {
    copied = path.join(evidenceDir, 'cursor-sailboat.png')
    await copyFile(path.join(rendersDir, best.name), copied)
  }
  const report = { sessionId: standIn.sessionId, host, code: result.code, fresh: ranked, copied }
  await writeFile(path.join(evidenceDir, 'cursor-sailboat.txt'), `${JSON.stringify(report, null, 2)}\n\n${result.output.slice(-6000)}\n`)
