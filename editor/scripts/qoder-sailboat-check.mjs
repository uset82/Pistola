...
const mcpPath = path.join(repoRoot, '.pistola/qoder-mcp.json')
...
const qoderBundle = () => {
  const bundle = path.join(process.env.APPDATA ?? '', 'npm/node_modules/@qoder-ai/qodercli/bundle/qodercli.js')
  if (!existsSync(bundle)) throw new Error(`qodercli.js was not found at ${bundle}`)
  return bundle
}

const runQoder = () => new Promise((resolve) => {
  const child = spawn(process.execPath, [
    ...
    '--strict-mcp-config',
    '--mcp-config', mcpPath,
    '--allowed-mcp-server-names', 'pistola',
...
const main = async () => {
  ...
  if (best && best.size > 10_000) {
    copied = path.join(evidenceDir, 'qoder-sailboat.png')
  ...
  await writeFile(path.join(evidenceDir, 'qoder-sailboat.txt'), `${JSON.stringify(report, null, 2)}\n\n${result.output.slice(-6000)}\n`)
