...
const mcpPath = path.join(repoRoot, '.pistola/claude-mcp.json')
...
const claudeCommand = () => {
  const exe = path.join(process.env.USERPROFILE ?? '', '.local/bin/claude.exe')
  if (!existsSync(exe)) throw new Error(`claude.exe was not found at ${exe}`)
  return exe
}

const runClaude = (sessionId) => new Promise((resolve) => {
...
const main = async () => {
  ...
  try {
    result = await runClaude(standIn.sessionId)
  } finally {
  ...
  for (const name of fresh) {
    ...
    ranked.push({ name, full, size: info.size })
  }
  ranked.sort((left, right) => right.size - left.size)
  let copied = null
  const best = ranked[0]
  if (best && best.size > 10_000) {
    copied = path.join(evidenceDir, 'claude-sailboat.png')
    await copyFile(best.full, copied)
  }
  const report = { sessionId: standIn.sessionId, code: result.code, fresh: ranked, copied }
  await writeFile(path.join(evidenceDir, 'claude-sailboat.txt'), `${JSON.stringify(report, null, 2)}\n\n${result.output.slice(-6000)}\n`)
